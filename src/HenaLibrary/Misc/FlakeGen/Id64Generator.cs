...
namespace FlakeGen
{
    ...
    public class Id64Generator : IIdGenerator<long>
    {
        ...
        public Id64Generator(int generatorId = 0, int sequence = 0, long customMask = -1L ^ (-1L << 63))
            : this(
        ...
        public Id64Generator(int workerId, int datacenterId, int sequence, long customMask)
        {
		...
		public long CustomMask { get; private set; }
        ...
        private long NextId()
        {
			...
			byte[] bytes = BitConverter.GetBytes(id ^ CustomMask);
			Swap(ref bytes[0], ref bytes[3]);
			Swap(ref bytes[1], ref bytes[4]);
			Swap(ref bytes[2], ref bytes[5]);
			long newId = BitConverter.ToInt64(bytes, 0);
			return newId;
		}
